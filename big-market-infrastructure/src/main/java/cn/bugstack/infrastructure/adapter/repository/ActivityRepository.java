...
public class ActivityRepository implements IActivityRepository {
    ...
    public void doSaveNoPayOrder(CreateQuotaOrderAggregate createOrderAggregate) {
        ...
        //
        try{
            // 以用户ID作为切分键，通过DB中间件进行路由切分【这样就保证了下面的操作，都是同一个链接下，也就保证了事务的特性】
            ...
            transactionTemplate.execute(status -> {
                try{
                    ...
                    log.error("写入订单记录，唯一索引冲突 userId：{}, activityId：{}, sku：{}", createOrderAggregate.getUserId(), createOrderAggregate.getActivityId(), createOrderAggregate.getActivityOrderEntity().getSku());
    ...
    public boolean subtractionActivitySkuStock(Long sku, String cacheKey, Date endDateTime) {
        ...
        // 2、加锁为了兜底，如果后续有恢复库存，手动处理等【运营是人来操作，会有这种情况发生，系统要做防护】，也不会超卖，因为所有的可用库存key，都被加锁了
        // 3、设置加锁时间为活动到期 + 延迟1天
        ...
        try{
            ...
            transactionTemplate.execute(status -> {
                try{
                    ...
                    status.setRollbackOnly();
                    log.error("写入参与订单记录，唯一索引冲突 userId：{}, activityId：{}", userId, activityId, e);
                    throw new AppException(ResponseCode.INDEX_DUP.getCode());
                }
               return 1;
            });
    ...
    public void updateOrder(DeliveryOrderEntity deliveryOrderEntity) {
        RLock lock = redisService.getLock(Constants.RedisKey.ACTIVITY_ACCOUNT_LOCK + deliveryOrderEntity.getUserId());
        try{
